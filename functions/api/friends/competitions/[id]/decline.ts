...
import { declineFriendsInvitation, getFriendsError } from "../../../../_shared/friends";
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    const result = await declineFriendsInvitation(context.env.DB, getParam(context.params.id), auth.user.id);
